...
/**
 * <p>
 * AiController class for the AlphaMTG AI variant.
 * </p>
 *
...
//    }
    ...
    // Card Memory likely not needed since we will be asking for entire game state
    // with each decision made
//    public AiCardMemory getCardMemory() {
//        return memory;
//    }

///////////// Lots of AI helper methods that are not required for AlphaMTG
    // used to predict if a creature will attack/block, unnecessary
...
//    }
    ...
    public CardCollection getCardsToDiscard(int min, final int max, final CardCollection validCards, final SpellAbility sa) {
        ...
        if (sa != null) {
            ...
            if (sa.hasParam("AnyNumber")) {
                if ("DiscardUncastableAndExcess".equals(sa.getParam("AILogic"))) {
                    ...
                    final int numLandsOTB = CardLists.count(inHand, Presets.LANDS);
                    int numOppInHand = 0;
        ...
        for (int i = 0; i < discardsLeft; i++) {
            ...
            final int numLandsInPlay = CardLists.count(player.getCardsIn(ZoneType.Battlefield), Presets.LANDS_PRODUCING_MANA);
            final CardCollection landsInHand = CardLists.filter(validCards, Presets.LANDS);
        ...
        for (final SpellAbility sa : ComputerUtilAbility.getOriginalAndAltCostAbilities(all, player)) {
            ...
            if (useLivingEnd) {
                if (sa.isCycling() && sa.canCastTiming(player) && player.getCardsIn(ZoneType.Library).size() >= 10) {
                    ...
                    } else if (CardLists.filter(player.getZone(ZoneType.Graveyard).getCards(), Presets.CREATURES).size() > 4) {
    ...
    public CardCollectionView cheatShuffle(CardCollectionView in) {
        ...
        CardCollection land = CardLists.filter(library, Presets.LANDS);
        for (Card c : land) {
            if (c.isLand()) {
                library.remove(c);
            }
